...
export default function AdminDashboard() {
  ...
  async function toggleQuiz(labId: string, currentStatus: boolean) {
    ...
    try {
      ...
      if (res.ok) {
        const labsRes = await fetch("/api/labs?activeOnly=true&subject=ITCS223")
        if (labsRes.ok) {
  ...
  async function toggleQuizSection() {
    ...
    try {
      const res = await fetch('/api/subjects/toggle-quiz-section', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ subjectCode: 'ITCS223', enabled: !quizSectionEnabled })
      })
  ...
  async function handleGradeSubmit(e: React.FormEvent) {
    ...
    try {
        ...
        if (res.ok) {
             ...
             try {
                const detailsRes = await fetch(`/api/scores?username=${studentId}&subject=ITCS223`)
                if (detailsRes.ok) {
                   ...
                   <div className="flex items-start gap-4">
                      ...
                      <div>
                         ...
                         {studentDetails ? (
                             ...
                             <p className="text-sm">Score updated for Student {lastSubmittedStudentId} in ITCS223 Sheet.</p>
                         )}
                      </div>
                   </div>
                </div>
            ...
            <form onSubmit={handleCreateLab} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-teal-500"
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-bold shadow-lg shadow-teal-500/30 transition-all disabled:opacity-50"
                >
