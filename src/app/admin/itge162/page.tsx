...
export default function AdminDashboard() {
  ...
  async function toggleQuiz(labId: number, currentStatus: boolean) {
    ...
    try {
      ...
      if (data.success) {
        // Refresh lab list
        const labsRes = await fetch("/api/labs?activeOnly=true&subject=ITGE162")
        if (labsRes.ok) {
        ...
        <div className="container mx-auto max-w-7xl flex h-16 items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-2 md:gap-8">
            <div className="flex items-center gap-2 md:gap-4">
              <div className="flex h-8 w-8 md:h-10 md:w-10 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-600 to-green-600 text-white font-bold shadow-lg shadow-emerald-500/30 text-xs">
                PS
              </div>
            ...
            <div className="hidden lg:flex items-center gap-4 md:gap-6">
              ...
              <a
                href="/admin/quiz-management?subject=ITGE162"
                className="text-sm font-medium text-slate-500 hover:text-emerald-600 transition-colors whitespace-nowrap"
              >
                Quiz Management
              </a>
            </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg">
            <div className="container mx-auto max-w-7xl px-4 py-4 space-y-2">
              <a
                href="/admin/dashboard"
                className="block px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors"
              >
                &larr; Hub
              </a>
              <div className="block px-4 py-2 text-sm font-medium text-teal-600 dark:text-teal-400 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
                Dashboard
              </div>
              <a
                href="/admin/quiz-management?subject=ITGE162"
