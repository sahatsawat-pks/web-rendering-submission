...
export default function AdminDashboard() {
  ...
  async function toggleQuiz(labId: string, currentStatus: boolean) {
    ...
    try {
      ...
      if (res.ok) {
        // Refresh labs list
        const labsRes = await fetch("/api/labs?subject=ITCS123")
        if (labsRes.ok) {
  ...
  async function toggleQuizSection() {
    ...
    try {
      const res = await fetch('/api/subjects/toggle-quiz-section', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ subjectCode: 'ITCS123', enabled: !quizSectionEnabled })
      })
  ...
  async function handleGradeSubmit(e: React.FormEvent, scoreType: 'lab' | 'challenge' | 'both') {
    ...
    try {
        ...
        if (res.ok) {
             ...
             try {
                const detailsRes = await fetch(`/api/scores?username=${studentId}&subject=ITCS123`)
                if (detailsRes.ok) {
        ...
        <div className="flex-1 space-y-8">
          ...
          <div className="animate-slide-up">
            <div className="flex items-center justify-between mb-2">
              <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 dark:text-slate-100">ITCS123 Dashboard</h1>
          ...
          <div className="glass-card p-8 animate-scale-in hover:shadow-2xl hover:shadow-orange-500/5 transition-all duration-300 border-white/40">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                ...
                {['Lecturer', 'Main Admin'].includes(role) && (                  <button onClick={() => setShowNewLabDialog(true)} className="px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-xs font-bold shadow-sm transition-colors flex items-center gap-1">
                ...
                {Array.from(new Map(labs.map(lab => [lab.labNumber, lab])).values()).map((lab) => {
                  ...
                  >
                    <div className={`flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center text-xl sm:text-2xl font-bold shadow-lg transition-transform duration-300 ${
                       lab.isActive
                        ? "bg-gradient-to-br from-orange-500 to-amber-500 text-white shadow-orange-500/20 group-hover:scale-105"
            ...
            <form onSubmit={handleCreateLab} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-orange-500"
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-bold shadow-lg shadow-orange-500/30 transition-all disabled:opacity-50"
                >
