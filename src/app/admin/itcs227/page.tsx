...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function fetchLabs() {
      try {
        const res = await fetch("/api/labs?subject=ITCS227")
        if (res.ok) {
    ...
    fetch("/api/student-prefixes?subject=ITCS227")
      .then(res => res.json())
      .then(data => {
  ...
  async function toggleQuizSection() {
    ...
    try {
      const res = await fetch('/api/subjects/toggle-quiz-section', {
        ...
        body: JSON.stringify({ subjectCode: 'ITCS227', enabled: !quizSectionEnabled })
      })
  ...
  async function handleGradeSubmit(e: React.FormEvent) {
    ...
    try {
        ...
        if (res.ok) {
             ...
             try {
                const detailsRes = await fetch(`/api/scores?username=${studentId}&subject=ITCS227`)
                if (detailsRes.ok) {
  ...
  async function toggleQuiz(labId: number, currentStatus: boolean) {
    ...
    try {
      ...
      if (data.success) {
        // Refresh lab list
        const labsRes = await fetch("/api/labs?subject=ITCS227")
        if (labsRes.ok) {
        ...
        <div className="flex-1 space-y-8">
          ...
          <div className="animate-slide-up">
            <div className="flex items-center justify-between mb-2">
              <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 dark:text-slate-100">ITCS227 Dashboard</h1>
          ...
          <div className="glass-card p-8 animate-scale-in hover:shadow-2xl hover:shadow-teal-500/5 transition-all duration-300 border-white/40">
            <div className="flex items-center justify-between mb-8">
              ...
              <a
                href="https://docs.google.com/spreadsheets/d/1LnPggDqEnvGZ7LSEhZZf0TSE8bnbX_3zuoZFeIpJD_g/edit?ouid=107284221226923478169&usp=sheets_home&ths=true"
                target="_blank"
                   ...
                   <div className="flex items-start gap-4">
                      ...
                      <div>
                         ...
                         {studentDetails ? (
                             ...
                             <p className="text-sm">Score updated for Student {lastSubmittedStudentId} in ITCS227 Sheet.</p>
                         )}
                      </div>
                   </div>
                </div>
          ...
          <div className="glass-card p-8 animate-scale-in hover:shadow-2xl hover:shadow-teal-500/5 transition-all duration-300 border-white/40">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                ...
                {['Lecturer', 'Main Admin'].includes(role) && (
                  <a href="/admin/itcs227/quiz" className="px-3 py-1.5 bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 rounded-lg text-xs font-medium border border-pink-200 dark:border-pink-800 shadow-sm hover:bg-pink-200 dark:hover:bg-pink-900/50 transition-colors">
                    Manage Quiz
                  </a>
                )}
