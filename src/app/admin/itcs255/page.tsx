...
export default function ITCS255AdminDashboard() {
  ...
  async function toggleQuiz(labId: number, currentStatus: boolean) {
    ...
    try {
      ...
      if (data.success) {
        // Refresh lab list
        const labsRes = await fetch("/api/labs?subject=ITCS255")
        if (labsRes.ok) {
                ...
                {labs.map((lab) => (
                  <div
                    ...
                    className={`flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5 p-4 sm:p-5 rounded-2xl border transition-all smooth-transition group ${
                      lab.isActive
                        ? "border-slate-100 dark:border-slate-700 bg-white/60 dark:bg-slate-800/60 hover:border-purple-300 dark:hover:border-purple-600 hover:shadow-lg hover:shadow-purple-500/5"
                  ...
                  >
                    <div className={`flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center text-xl sm:text-2xl font-bold shadow-lg transition-transform duration-300 ${
                       lab.isActive
                        ? "bg-gradient-to-br from-purple-500 to-pink-500 text-white shadow-purple-500/20 group-hover:scale-105"
