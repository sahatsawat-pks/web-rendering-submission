...
export default function ITCS255AdminDashboard() {
  ...
  async function toggleQuizSection() {
    ...
    try {
      const res = await fetch('/api/subjects/toggle-quiz-section', {
        ...
        body: JSON.stringify({ subjectCode: 'ITCS255', enabled: !quizSectionEnabled })
      })
  ...
  async function toggleQuiz(labId: number, currentStatus: boolean) {
    ...
    try {
      ...
      if (data.success) {
        // Refresh lab list
        const labsRes = await fetch("/api/labs?activeOnly=true&subject=ITCS255")
        if (labsRes.ok) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-pink-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-950 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 -left-4 w-96 h-96 bg-purple-300 rounded-full mix-blend-multiply filter blur-xl opacity-5 animate-float"></div>
        <div
          className="absolute bottom-0 -right-4 w-96 h-96 bg-pink-300 rounded-full mix-blend-multiply filter blur-xl opacity-5 animate-float"
        ...
        <div className="container mx-auto max-w-7xl flex h-16 items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-2 md:gap-8">
            <div className="flex items-center gap-2 md:gap-4">
              <div className="flex h-8 w-8 md:h-10 md:w-10 items-center justify-center rounded-xl bg-gradient-to-br from-purple-600 to-pink-600 text-white font-bold shadow-lg shadow-purple-500/30 text-xs">
                DB
              </div>
            ...
            <div className="hidden lg:flex items-center gap-4 md:gap-6">
              <a
                 ...
                 className="text-sm font-medium text-slate-500 hover:text-purple-600 transition-colors whitespace-nowrap"
              >
                &larr; Hub
              </a>
              <span
                className="text-sm font-medium text-purple-600 dark:text-purple-400 border-b-2 border-purple-600 dark:border-purple-400 h-16 flex items-center px-1 whitespace-nowrap"
              >
                Dashboard
              </span>
              <a
                href="/admin/quiz-management?subject=ITCS255"
                className="text-sm font-medium text-slate-500 hover:text-purple-600 transition-colors whitespace-nowrap"
              >
                Quiz Management
              </a>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg">
            <div className="container mx-auto max-w-7xl px-4 py-4 space-y-2">
              <a
                href="/admin/dashboard"
                className="block px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded-lg transition-colors"
              >
                &larr; Hub
              </a>
              <div className="block px-4 py-2 text-sm font-medium text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                Dashboard
              </div>
              <a
                href="/admin/quiz-management?subject=ITCS255"
            ...
            {loading ? (
              ...
              <div className="grid gap-4">
                {labs.map((lab) => (
                  <div
                    key={lab.id}
                    className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5 p-4 sm:p-5 rounded-2xl border border-slate-100 dark:border-slate-700 bg-white/60 dark:bg-slate-800/60 hover:border-purple-300 dark:hover:border-purple-600 hover:shadow-lg hover:shadow-purple-500/5 transition-all smooth-transition group"
                  >
                    <div className="flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl flex items-center justify-center text-xl sm:text-2xl font-bold text-white shadow-lg shadow-purple-500/20 group-hover:scale-105 transition-transform duration-300">
                      {lab.labNumber}
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap sm:flex-nowrap">
                      ...
                      {(role === 'Lecturer' || username === 'kanzaki_aito') && (
                        <a
                          href="/admin/labs"
                          className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 rounded-lg shadow-md shadow-purple-500/30 transition-all btn-hover-lift"
                        >
                          Manage
                        </a>
                      )}
            ...
            <form onSubmit={handleCreateLab} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-purple-500"
                  placeholder="e.g., Database Basics"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-bold shadow-lg shadow-purple-500/30 transition-all disabled:opacity-50"
                >
