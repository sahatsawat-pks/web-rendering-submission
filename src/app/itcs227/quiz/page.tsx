...
export default function ITCS227QuizPage() {
  ...
  const fetchLabsWithQuiz = async () => {
    try {
      const res = await fetch('/api/quiz?action=list_labs&subject=ITCS227')
      if (res.ok) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 flex items-center justify-center">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-10 w-10 border-4 border-indigo-200 dark:border-indigo-800 border-t-indigo-600"></div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      {/* Header */}
        ...
        <div className="container mx-auto max-w-7xl flex h-16 items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-indigo-600 dark:text-indigo-400 font-medium hidden sm:block">Check Your Understanding</p>
            </div>
          </div>
          <ModeToggle />
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-indigo-100 dark:border-slate-700 p-6 mb-6">
          ...
          <div className="space-y-4">
            {labs.map(lab => (
              <div
                ...
                onClick={() => router.push(`/itcs227/quiz/${lab.labNumber}`)}
              >
