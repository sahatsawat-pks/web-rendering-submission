...
export default function ITGE162QuizPage() {
  ...
  const fetchLabsWithQuiz = async () => {
    try {
      const res = await fetch('/api/quiz?action=list_labs&subject=ITGE162')
      if (res.ok) {
          ...
          <div className="space-y-4">
            {labs.map(lab => (
              <div
                ...
                onClick={() => router.push(`/itge162/quiz/${lab.labNumber}`)}
              >
