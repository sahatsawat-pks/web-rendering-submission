...
export default function ITDS283QuizPage() {
  ...
  const fetchLabsWithQuiz = async () => {
    try {
      const res = await fetch('/api/quiz?action=list_labs&subject=ITDS283')
      if (res.ok) {
          ...
          <div className="space-y-4">
            {labs.map(lab => (
              <div
                ...
                onClick={() => router.push(`/itds283/quiz/${lab.labNumber}`)}
              >
