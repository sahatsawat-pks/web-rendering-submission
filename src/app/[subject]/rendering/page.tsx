...
export default function SubjectRenderingPage() {
  ...
  // Test Runner State
  // Checking if active lab is a test case lab (generic logic needed?)
  // Original code hardcoded ["5", "05", "6", "06"].
  // We should rely on configuration or API to tell us if a lab has tests.
  // For now, I'll keep the hardcoded check but maybe I can make it cleaner later.
  // Actually, I can check if 'testRunner' is available in the lab object if the API returns it.
  ...
  useEffect(() => {
    ...
    loadLabs()
  }, [subject, router, config, loadingConfig])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/fetch-submission", {
        ...
        body: JSON.stringify({ username, labNumber, subject }),
      })
  ...
  const runTests = async () => {
    ...
    try {
      const labRes = await fetch(`/api/labs?subject=${subject}&labNumber=${labNumber}`);
      if (!labRes.ok) throw new Error("Failed to fetch lab configuration");
      
      const labData = await labRes.json();
      if (!labData.success || !labData.labs || labData.labs.length === 0) throw new Error("Lab not found");
      ...
      if (testCases.length === 0) {
        setTestResults({ passed: 0, total: 0, cases: [] });
      ...
      setTestResults({ passed: 0, total: testCases.length, cases: initialCases });
      ...
      if (!testRes.ok) throw new Error("Failed to execute tests");
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${config.bgGradient} relative overflow-hidden`}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute top-0 -left-4 w-96 h-96 ${config.accentColor.replace('text-', 'bg-').replace('600', '300')} rounded-full mix-blend-multiply dark:mix-blend-soft-light filter blur-3xl opacity-20 dark:opacity-10 animate-float`}></div>
      </div>
      ...
      <nav className="sticky top-0 z-50 w-full glass border-b border-white/30 dark:border-slate-700/50 shadow-lg">
        <div className="container mx-auto max-w-7xl flex h-16 items-center justify-between px-6">
          <div className="flex items-center gap-3">
            ...
            <img src="/logo.png" alt="Logo" className={`h-11 w-11 rounded-xl shadow-lg ${config.shadowColor}`} />
            <div>
              <span className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-100">
                Submission Viewer
              </span>
              <p className={`text-xs ${config.accentColor} font-medium`}>{config.title}</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <a
              href="/admin/login"
              className={`px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:${config.accentColor} transition-colors rounded-xl hover:bg-white/80 dark:hover:bg-slate-700/60 flex items-center gap-2`}
            >
            ...
            <div className="flex flex-1 overflow-hidden relative">
              ...
              <div className="flex-1 bg-white dark:bg-slate-900 relative flex flex-col">
                ...
                <div className="flex-1 flex items-start justify-center p-4 overflow-auto bg-slate-100 dark:bg-slate-900">
                  <div
                    className="relative bg-white dark:bg-slate-950 shadow-2xl transition-all duration-300"
                    style={{
