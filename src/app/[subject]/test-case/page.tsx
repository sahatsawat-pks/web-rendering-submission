...
const DEFAULT_SQL_CODE = `-- Write your SQL query here
...
export default function SubjectTestRunner() {
  ...
  useEffect(() => {
    ...
    // Set default code based on runner type
    if (config?.testRunnerType === 'python') setCode(DEFAULT_PYTHON_CODE)
    else if (config?.testRunnerType === 'java') setCode(DEFAULT_JAVA_CODE)
    else if (config?.testRunnerType === 'sql') setCode(DEFAULT_SQL_CODE)

    async function loadLabs() {
      try {
        const res = await fetch(`/api/labs?activeOnly=true&subject=${subject}`)
        if (res.ok) {
  ...
  useEffect(() => {
    if (labNumber && labs.length > 0) {
      // Filter logic: Java uses labType, Python/others might not
      let currentLab;
      if (config?.testRunnerType === 'java') {
          currentLab = labs.find(l => l.labNumber === labNumber && (l.labType || 'Lab') === labTypeFilter)
      } else {
          currentLab = labs.find(l => l.labNumber === labNumber)
      }
  ...
  }, [labNumber, labs, labTypeFilter, config?.testRunnerType])

  const getApiEndpoint = () => {
      switch (config?.testRunnerType) {
          case 'python': return '/api/run-python'
          case 'java': return '/api/run-java'
          case 'sql': return '/api/run-sql' // Placeholder if SQL runner API exists
          default: return '/api/run-python'
      }
  }
      ...
      try {
        const response = await fetch(getApiEndpoint(), {
            ...
            body: JSON.stringify({
                code,
                input: customInput,
                labNumber: labNumber // SQL runner might need this context
            })
        })
    ...
    try {
      const response = await fetch(getApiEndpoint(), {
        ...
        body: JSON.stringify({
          ...
          verificationCode: test.verificationCode,
           // Java runner expects 'input', Python logic was updated to support verificationCode.
           // Java logic in reference file did not send verificationCode but maybe it should?
           // I'll send it regardless, API should ignore if unused.
        })
      })
      ...
      setTestCases(prev => prev.map(t => {
        ...
        if (result.error && !result.output) {
          ...
          // Fail if there is any error output (including verification errors for Python)
    ...
    for (let i = 0; i < testsToRun.length; i++) {
        ...
        try {
            ...
            setTestCases(prev => prev.map(t => {
                ...
                if (result.error && !result.output) {
                     updatedTest.status = 'fail'
                     updatedTest.actualOutput = result.error 
                     updatedTest.errorMessage = result.error
                } else {
                    const rawActual = result.output || ''
                    const rawExpected = updatedTest.expectedOutput
  ...
  const langConfig = {
      python: { icon: 'PY', color: 'bg-blue-600', hoverBlob: 'bg-blue-300' },
      java: { icon: 'JV', color: 'bg-orange-600', hoverBlob: 'bg-orange-300' },
      sql: { icon: 'SQL', color: 'bg-purple-600', hoverBlob: 'bg-purple-300' }
  }[config.testRunnerType || 'python'] || { icon: 'Code', color: 'bg-slate-600', hoverBlob: 'bg-slate-300' }
        ...
        <div className="container mx-auto max-w-7xl flex h-14 items-center justify-between px-4">
          <div className="flex items-center gap-3">
            ...
            >
             ...
             <div className={`flex h-8 w-8 items-center justify-center rounded ${langConfig.color} text-white font-bold`}>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="bg-slate-900 px-4 py-3 border-b border-slate-800 space-y-2 md:space-y-3">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <select
                        ...
                        className={`flex-1 bg-slate-800 text-slate-200 px-3 py-1.5 rounded text-xs border border-slate-700 focus:outline-none focus:ring-2 focus:ring-opacity-50 focus:ring-${langConfig.color.replace('bg-', '')}`}
                    >
                        <option value="">Choose Lab</option>
                        {labs
                            .filter(lab => config.testRunnerType !== 'java' || (lab.labType || 'Lab') === labTypeFilter)
                            .map(lab => (
            ...
            <div className="bg-slate-900 px-4 py-2 border-b border-slate-800 flex justify-between items-center">
                <span className="text-xs text-slate-400 flex items-center gap-2">
                    ...
                    {(() => {
                        ...
                        } else if (config.testRunnerType === 'python') {
                            return 'solution.py';
                        } else {
                            return 'query.sql';
                        }
            ...
            <textarea
                ...
                spellCheck={false}
                placeholder={`Paste your ${config.testRunnerLabel || 'code'} here...`}
            />
        </div>
                    ...
                    <div key={taskName}>
                      {taskName !== '__no_task__' && (
                        <div className="mb-2 px-2">
                          <div className={`text-xs font-bold ${config.testRunnerType === 'python' ? 'text-blue-400' : 'text-orange-400'} uppercase tracking-wide`}>{taskName}</div>
                          <div className={`h-px ${config.testRunnerType === 'python' ? 'bg-blue-500/30' : 'bg-orange-500/30'} mt-1`}></div>
                        </div>
                      )}
                      {tests.map((test) => (
